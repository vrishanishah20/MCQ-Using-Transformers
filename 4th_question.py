...
def read_corpus(filename, tokenizer):
    print(f"\nReading and tokenizing corpus: {filename}")
    seq = []
...
# Modified Attention (Euclidean Distance)
...
def train_model(model, opt, train_data, valid_data):
    print("\nStarting training...")
    os.makedirs('4th_question', exist_ok=True)
    ...
    np.savetxt('4th_question/train_loss.txt', np.array(train_losses))
    np.savetxt('4th_question/val_loss.txt', np.array(val_losses))

    plt.plot(train_losses, label='Train')
    plt.plot(val_losses, label='Validation')
    plt.legend()
    plt.title('Training and Validation Loss (Euclidean)')
    plt.savefig('4th_question/loss_curve.png')
    plt.close()

    torch.save(model.state_dict(), '4th_question/model_final.pt')
...
def test_model(model, opt, test_data):
    ...
    print(f"Test Perplexity (Euclidean Attention): {perplexity:.2f}")

def main():
    print("Setting up parser and config...")
    ...
    print(f"Device in use: {opt.device}")

    print("Loading tokenizer and datasets...")
    tokenizer = GPT2TokenizerFast.from_pretrained("gpt2")
    train_data = read_corpus('wiki2.train.txt', tokenizer)
    valid_data = read_corpus('wiki2.valid.txt', tokenizer)
    test_data = read_corpus('wiki2.test.txt', tokenizer)

    print("\nBuilding model...")
    model = TransformerGPT2(vocab_size=50257, d_model=opt.d_model, N=opt.n_layers, heads=opt.heads, dropout=opt.dropout)
    model.to(opt.device)

    train_model(model, opt, train_data, valid_data)

    print("\nLoading saved model for testing...")
    model.load_state_dict(torch.load('4th_question/model_final.pt'))
